...
const fallbackContent: DailyContent = {
  id: 1,
  day: 1,
  title: "Como escolher o fotógrafo ideal para o seu casamento sem arrependimentos",
  body: [
  ...
  cta: "Guarde este post para quando começar a procurar fornecedores."
};
      ...
      <section className="agenda" aria-label="Agenda do dia">
        <article className="today">
          ...
          <div className="progress-note">
            O conteúdo é desbloqueado dia após dia para manter a sua presença ativa sem decisões.
          </div>
        ...
        <aside className="upcoming" aria-label="Próximos dias">
          <div className="next-title">Próximos dias</div>
          {upcomingDays.map((item) => (
            <div className="locked-day" key={item.day}>
              <strong>Dia {item.day} <span>🔒</span></strong>
              <span>{item.label}</span>
            </div>
          ))}
          <div className="note">Abra. Copie. Publique. E volte ao que realmente precisa da sua atenção.</div>
        </aside>
      </section>
    </main>
  );
